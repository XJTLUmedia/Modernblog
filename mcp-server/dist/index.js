...
async function executeTool(toolDef, args) {
    ...
    // Resolve path
    const path = typeof toolDef.path === "function" ? toolDef.path(args) : toolDef.path;
    let result;
...
function isSuccessfulLogin(result) {
    if (typeof result !== "object" || result === null)
        return false;
    const r = result;
    return r.success === true && typeof r.user === "object" && r.user !== null;
}
function isAuthCheckSuccess(result) {
    if (typeof result !== "object" || result === null)
        return false;
    const r = result;
...
for (const toolDef of ALL_TOOLS) {
    ...
    server.tool(toolDef.name, toolDef.description, shape, async (args) => {
        try {
...
server.resource("api-overview", "blog://api-overview", async (uri) => ({
    contents: [
        {
            uri: uri.href,
